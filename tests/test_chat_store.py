...
def test_format_generated_in() -> None:
    ...
    assert (
        format_generated_in(95, model_name="Mistral-7B-Instruct-v0.3-Q5_K_M")
        == "Generated by Mistral-7B-Instruct-v0.3-Q5_K_M in 1 min 35s"
    )
    assert format_generated_in(12, model_name="  ") == "Generated in 12s"
    assert format_generated_in(None, model_name="Mistral") == ""
...
def test_add_message_can_store_model_name() -> None:
    thread = empty_thread()
    stored = add_message(
        thread,
        "assistant",
        "Ready.",
        elapsed_seconds=42,
        model_name="  Qwen3.5-9B-Q5_K_M  ",
    )
    assert stored["model_name"] == "Qwen3.5-9B-Q5_K_M"
    assert "model_name" not in add_message(thread, "assistant", "Lookup.")
