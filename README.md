...
a simpler workbook. Users never touch Python or a terminal.

**Reliability does not depend on the local model.** On attach, the app
scans the files and caches an inventory. Many later questions are
answered from that cache **with no AI generate**. The model is used
...
reuse that cache. A Workbook library (reuse a pack across chats) comes
later.

## How files are read

Accepted types: **`.xlsx`**, **`.xlsm`**, **`.xls`**, **`.csv`**. Attach
once per chat (paperclip, or the folder picker for linked workbooks).
The GGUF never receives the raw file. Replace the pack if the source
workbook changes.
...
`For totals, counts, and “which group is largest” on the data itself, please attach the table as a CSV.`

If a CSV **has** a card but the column is not on it:

`That table fact is not on the inventory stats card.`  
`Available columns: …`

A **group total** (sum of all rows) is never the same as the largest
**single row**. Both can name the same group; they often do not.
...
| --- | --- |
| Answer appears at once. No spinner. No “Generated in …” | The app quoted the **cached inventory** (formulas, links, or table facts). The local model is not used, even if it is Ready. |
| Spinner, wait copy, then “Generated by … in 2 min …” | The **local model** that was loaded wrote the reply. You must click **Load model** and wait for Ready first. |

**Load model is not required** for inventory or table-fact questions.
It **is** required for explanations and suggested formulas that do not
name a `Sheet!A1` cell.
...
triggers. They are not case-sensitive. Formula lookups run **before**
table facts, so `Which file does Rates read?` is never treated as
...
If the question is **not** one of the six fact patterns, and a model
...
| You type | Path | Typical reply |
| --- | --- | --- |
| (inventory turn after attach) | App | Full row count. All low-cardinality names. Top-5 **sums**. Largest **single row** labelled as not a total. |
| `How many rows are in the file?` | Inventory, instant | The full row count from the card. |
| `List the agencies` | Inventory, instant | Every distinct agency on the card, including names after the sample window. |
| `Which agency costs the most?` | Inventory, instant | The group with the largest **sum of all rows**, then the largest single row labelled separately. |
...
End-user machines will **not** need Python in the final product.
Closed-beta testers get a **separate folder** assembled into
`release/ChatWithExcelFile/` (gitignored). Developers do not put
tester scripts at the repo root. See `tools/beta/`.
...
├── outputs/
├── tools/beta/      # recipe that fills gitignored release/ChatWithExcelFile/
└── tests/
```
...
Closed-beta testers do **not** use this repo tree. Zip
`release/ChatWithExcelFile/` after `tools\beta\assemble.bat`.
