...
| 6.5 Tabular facts | Implemented | Compact stats card at attach (full row count, low-cardinality values, numeric min/max/sum, year range, top-5 group sums, separate max row). List / count / min-max-sum / “which group is largest” are instant lookups. Fail closed if the fact is not on the card. No RAG. The model still does not compute numbers. |
